...
fn process_instruction(
    ...
    if system_program::check_id(program_id) { //### is it possible to pass the wrong program id and still be able to call A particular program?
    ...
    let accounts_iter = &mut accounts.iter(); //### taking a mutable reference to the accounts so that we can mutate the values we get from the iterator. Iterating over vectors is safer than indexing

    let _payer = next_account_info(accounts_iter)?; //## the first account is the payer, it is not being used in the program that is why there is an underscore on the variable name 
